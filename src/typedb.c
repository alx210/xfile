...
#ifdef DB_TEST
static void print_rec(struct file_type_rec *rec);

int main(int argc, char **argv)
{
	char *db_file="test.db";
	struct file_type_db db;
	int i;
	
	if(argc>1) db_file = argv[1];
	
	memset(&db,0,sizeof(struct file_type_db));
	if(db_parse_file(db_file, &db, 0)){
...
static void print_rec(struct file_type_rec *rec)
{
	...
	printf("\nTYPE: %s\n"
		"\tICON: %s\n\tMIME: %s\n",
		rec->name,
		rec->icon_name ? rec->icon_name : "(none)",
		rec->mime ? rec->mime : "(none)");
	if(rec->name_pat){
		puts("\tNAME PATTERNS:");
		for(i=0; i<rec->nname_pat; i++)
			printf("\t\t%d: %s\n", i, rec->name_pat[i]);
	...
	if(rec->content_pat){
		puts("\tCONTENT PATTERNS:");
		for(i=0; i<rec->ncontent_pat; i++){
			printf("\t\tOffset: %ld\tLength: %ld\tData: %s\n",
				rec->content_pat[i].offset,
				rec->content_pat[i].data_len,
				rec->content_pat[i].data);
		}
	}

	if(rec->actions){
		printf("\tACTIONS:\n");
		for(i=0; i<rec->nactions; i++){
			printf("\t\tName: %s\tCommand: %s\n",
				rec->actions[i].title,
				rec->actions[i].command);
		}
	}
